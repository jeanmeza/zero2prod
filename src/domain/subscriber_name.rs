...
impl SubscriberName {
    ...
    pub fn parse(s: String) -> Result<SubscriberName, String> {
        // `.trim()` returns a view over the input `s` without traiing withspace-like characters.
        // `is_empty()` checks if the view contains any character.
        let is_empty_or_whitespace = s.trim().is_empty();

        // a grapheme is defined by the Unicode standard as a "user-perceived" character: `è` is a
        // single grapheme, but it is composed of two characters (`e` and ```).
...
/// This caller gets a shared reference to the inner string.
/// This gives the caller **read-only** access, they havo no way to compromise our invariants.
...
mod tests {
    ...
    fn names_containing_an_invalid_character_are_rejected() {
        for name in &['/', '(', ')', '"', '<', '>', '\\', '{', '}'] {
            let name = format!("a name with a {} in it", name);
            assert_err!(SubscriberName::parse(name));
        }
    }
