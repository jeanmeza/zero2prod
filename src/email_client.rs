...
impl EmailClient {
    ...
    pub async fn send_email(
        ...
        // `.json()` also sets the `Content-Type` header to `application/json`
        let builder = self
            .http_client
            .post(url)
            .header(
...
mod tests {
    ...
    impl wiremock::Match for SendEmailBodyMatcher {
        fn matches(&self, request: &Request) -> bool {
            ...
            if let Ok(body) = result {
                // Check that all the mandatory fields are populated without inspecting the field
                // values
    ...
    async fn send_email_succeds_if_the_server_returns_200() {
        let mock_server = MockServer::start().await;
        let sender = SubscriberEmail::parse(SafeEmail().fake()).unwrap();
        let email_clinent = EmailClient::new(mock_server.uri(), sender, Secret::new(Faker.fake()));
        ...
        // We add the base minimum needed to trigger the path we want to test in  `send_email`.
        Mock::given(any())
            .respond_with(ResponseTemplate::new(200))
            .expect(1)
            .mount(&mock_server)
    ...
    #[tokio::test]
    async fn send_emailt_times_out_if_the_server_takes_too_long() {
        let mock_server = MockServer::start().await;
        let sender = SubscriberEmail::parse(SafeEmail().fake()).unwrap();
        let email_client = EmailClient::new(mock_server.uri(), sender, Secret::new(Faker.fake()));
